...
#undef min
#undef max
//#include <osgEarthUtil/EarthManipulator2>

#include <osgEarth/GeoMath>
#include <osgEarth/TerrainEngineNode>
#include <osgViewer/View>

#include "EarthManipulator2"
#define LC "[EarthManip] "

using namespace osgEarth::Util_New;
...
namespace
{
    ...
    {
        ManipTerrainCallback(EarthManipulator2* manip) : _manip(manip) { }
        void onTileAdded(const TileKey& key, osg::Node* graph, TerrainCallbackContext& context)
        {
            osg::ref_ptr<EarthManipulator2> safe;
            if ( _manip.lock(safe) )
            {
                safe->handleTileAdded(key, graph, context);
            }
        }
        osg::observer_ptr<EarthManipulator2> _manip;
    };
}



EarthManipulator2::Action::Action( ActionType type, const ActionOptions& options ) :
_type( type ),
_options( options )
{
    init();
...
EarthManipulator2::Action::Action( ActionType type ) :
_type( type )
{
    init();
}
...
EarthManipulator2::Action::Action( const Action& rhs ) :
_type( rhs._type ),
_dir( rhs._dir ),
...
bool
EarthManipulator2::Action::getBoolOption( int option, bool defaultValue ) const
{
...
int
EarthManipulator2::Action::getIntOption( int option, int defaultValue ) const
{
    for(ActionOptions::const_iterator i = _options.begin(); i != _options.end(); i++ ) {
        if ( i->option() == option )
            return i->intValue();
...
double
EarthManipulator2::Action::getDoubleOption( int option, double defaultValue ) const
{
    for(ActionOptions::const_iterator i = _options.begin(); i != _options.end(); i++ ) {
        if ( i->option() == option )
...
EarthManipulator2::Action EarthManipulator2::NullAction( EarthManipulator2::ACTION_NULL );
...
EarthManipulator2::Settings::Settings() :
osg::Referenced                 (),
Revisioned                      (),
...
EarthManipulator2::Settings::Settings( const EarthManipulator2::Settings& rhs ) :
osg::Referenced( rhs ),
Revisioned     ( rhs ),
...
void
EarthManipulator2::Settings::apply(osg::ArgumentParser& args)
{
    bool   boolval;
    double doubleval;
...
void
EarthManipulator2::Settings::expandSpec( const InputSpec& input, InputSpecs& output ) const
{
...
void
EarthManipulator2::Settings::bind( const InputSpec& spec, const Action& action )
{
    InputSpecs specs;
...
void
EarthManipulator2::Settings::bindMouse(ActionType actionType,
...
void
EarthManipulator2::Settings::bindMouseClick(ActionType action,
                                           int button_mask, int modkey_mask,
                                           const ActionOptions& options)
{
    bind(
...
void
EarthManipulator2::Settings::bindMouseDoubleClick(ActionType action,
                                                 int button_mask, int modkey_mask,
                                                 const ActionOptions& options)
{
    bind(
...
void
EarthManipulator2::Settings::bindKey(ActionType action,
...
void
EarthManipulator2::Settings::bindScroll(ActionType action, int scrolling_motion,
...
void
EarthManipulator2::Settings::bindPinch(ActionType action, const ActionOptions& options)
{
    bind(
        InputSpec( EarthManipulator2::EVENT_MULTI_PINCH, 0, 0 ),
        Action( action, options ) );
}

void
EarthManipulator2::Settings::bindTwist(ActionType action, const ActionOptions& options)
{
    bind(
         InputSpec( EarthManipulator2::EVENT_MULTI_TWIST, 0, 0 ),
         Action( action, options ) );
}

void
EarthManipulator2::Settings::bindMultiDrag(ActionType action, const ActionOptions& options)
{
    bind(
        InputSpec( EarthManipulator2::EVENT_MULTI_DRAG, 0, 0 ),
        Action( action, options ) );
}

const EarthManipulator2::Action&
EarthManipulator2::Settings::getAction(int event_type, int input_mask, int modkey_mask) const
{
...
void
EarthManipulator2::Settings::setMinMaxPitch( double min_pitch, double max_pitch )
{
...
void
EarthManipulator2::Settings::setMaxOffset(double max_x_offset, double max_y_offset)
{
...
void
EarthManipulator2::Settings::setMinMaxDistance( double min_distance, double max_distance)
{
...
void
EarthManipulator2::Settings::setArcViewpointTransitions( bool value )
{
...
void
EarthManipulator2::Settings::setAutoViewpointDurationEnabled( bool value )
{
...
void
EarthManipulator2::Settings::setAutoViewpointDurationLimits( double minSeconds, double maxSeconds )
{
...
EarthManipulator2::EarthManipulator2() :
...
EarthManipulator2::EarthManipulator2(osg::ArgumentParser& args) :
osgGA::CameraManipulator()
{
    ctor_init();
    reinitialize();
...
EarthManipulator2::EarthManipulator2( const EarthManipulator2& rhs ) :
...
void
EarthManipulator2::configureDefaultSettings()
{
    ...
    // 添加鼠标旋转操作 add by szh
    options.clear();
    options.add(OPTION_MOUSE_ROTATE_SPEED,0.2);
    _settings->bindMouseDoubleClick(ACTION_MOUSE_ROTATE, osgGA::GUIEventAdapter::LEFT_MOUSE_BUTTON,osgGA::GUIEventAdapter::MODKEY_SHIFT,options);
    _settings->bindMouseDoubleClick(ACTION_MOUSE_ROTATE, osgGA::GUIEventAdapter::RIGHT_MOUSE_BUTTON,osgGA::GUIEventAdapter::MODKEY_SHIFT,options);
}

void
EarthManipulator2::applySettings( Settings* settings )
{
    if ( settings )
    {
        _settings = settings;
...
EarthManipulator2::Settings*
EarthManipulator2::getSettings() const
{
    return _settings.get();
}

void
EarthManipulator2::reinitialize()
{
    _mouserotate = false;
    _distance = 1.0;
    _viewOffset.set(0,0);
...
void
EarthManipulator2::handleTileAdded(const TileKey& key, osg::Node* graph, TerrainCallbackContext& context)
{
...
bool
EarthManipulator2::createLocalCoordFrame( const osg::Vec3d& worldPos, osg::CoordinateFrame& out_frame ) const
{
    if ( _srs.valid() )
    {
        osg::Vec3d mapPos;
...
void
EarthManipulator2::setCenter( const osg::Vec3d& worldPos )
{
    _center = worldPos;
...
void
EarthManipulator2::setNode(osg::Node* node)
{
...
osg::Node*
EarthManipulator2::getNode()
{
    return _node.get();
}

osg::Matrixd
EarthManipulator2::getRotation(const osg::Vec3d& point) const
{
...
osg::Quat
EarthManipulator2::computeCenterRotation(const osg::Vec3d& point) const
{
...
Viewpoint
EarthManipulator2::getViewpoint() const
{
    Viewpoint vp;
...
void
EarthManipulator2::setViewpoint(const Viewpoint& vp, double duration_seconds)
{
...
double
EarthManipulator2::setViewpointFrame(double time_s)
{
...
void
EarthManipulator2::setLookAt(const osg::Vec3d& center,
                            double            azim,
                            double            pitch,
                            double            range,
...
bool
EarthManipulator2::isSettingViewpoint() const
{
    return _setVP0.isSet() && _setVP1.isSet();
}

void
EarthManipulator2::clearViewpoint()
{
...
bool
EarthManipulator2::intersect(const osg::Vec3d& start, const osg::Vec3d& end, osg::Vec3d& intersection, osg::Vec3d& normal) const
{
...
bool
EarthManipulator2::intersectLookVector(osg::Vec3d& out_eye,
...
void
EarthManipulator2::home(double unused)
{
    handleAction( ACTION_HOME, 0, 0, 0 );
}

void
EarthManipulator2::home(const osgGA::GUIEventAdapter& ,osgGA::GUIActionAdapter& us)
{
    home( 0.0 );
    us.requestRedraw();
}
...
void
EarthManipulator2::computeHomePosition()
{
    if( getNode() )
    {
...
void
EarthManipulator2::init(const osgGA::GUIEventAdapter&, osgGA::GUIActionAdapter& )
{
    flushMouseEventStack();
}
...
void
EarthManipulator2::getUsage(osg::ApplicationUsage& usage) const
{
}

void
EarthManipulator2::resetMouse( osgGA::GUIActionAdapter& aa, bool flushEventStack )
{
    if (flushEventStack)
      flushMouseEventStack();
...
void
EarthManipulator2::updateProjection(osg::Camera* eventCamera)
{
...
bool
EarthManipulator2::handle(const osgGA::GUIEventAdapter& ea, osgGA::GUIActionAdapter& aa)
{
    bool handled = false;
        ...
        switch( ea.getEventType() )
        {
            case osgGA::GUIEventAdapter::PUSH:
                ...
                if(_mouserotate) 
                {
                        ...
                        _viewOffset -= osg::Vec2(_moveoffset.x(), _moveoffset.y());
                }
                break;
...
void
EarthManipulator2::updateTether(double t)
{
...
bool
EarthManipulator2::isMouseMoving()
{
...
bool
EarthManipulator2::isMouseClick( const osgGA::GUIEventAdapter* mouse_up_event ) const
{
...
void
EarthManipulator2::flushMouseEventStack()
{
    _ga_t1 = NULL;
    _ga_t0 = NULL;
    //_touchPointQueue.clear();
...
void
EarthManipulator2::addMouseEvent(const osgGA::GUIEventAdapter& ea)
{
    _ga_t1 = _ga_t0;
    _ga_t0 = &ea;
    //_touchPointQueue.clear();
...
void
EarthManipulator2::addTouchEvents(const osgGA::GUIEventAdapter& ea)
{
    _ga_t1 = _ga_t0;
    _ga_t0 = &ea;
...
bool
EarthManipulator2::parseTouchEvents( TouchEvents& output )
{
...
void
EarthManipulator2::setByMatrix(const osg::Matrixd& matrix)
{
    if (!established())
        return;
...
osg::Matrixd
EarthManipulator2::getMatrix() const
{
    return getWorldMatrix() * _mapNodeFrame;
}

osg::Matrixd
EarthManipulator2::getWorldMatrix() const
{
...
osg::Matrixd
EarthManipulator2::getInverseMatrix() const
{
    return _mapNodeFrameInverse * getWorldInverseMatrix();
}

osg::Matrixd
EarthManipulator2::getWorldInverseMatrix() const
{
...
void
EarthManipulator2::updateCamera(osg::Camera& camera)
{
    // time exactly now
...
void
EarthManipulator2::setUpdateCameraNodeVisitor(osg::NodeVisitor* nv)
{
    _updateCameraNodeVisitor = nv;
}

void
EarthManipulator2::setByLookAt(const osg::Vec3d& eye,const osg::Vec3d& center,const osg::Vec3d& up)
{
    osg::ref_ptr<osg::Node> safeNode;
    if ( _node.lock(safeNode) )
    {
...
void
EarthManipulator2::setByLookAtRaw(const osg::Vec3d& eye,const osg::Vec3d& center,const osg::Vec3d& up)
{
    // compute rotation matrix
    osg::Vec3d lv(center-eye);
    setDistance( lv.length() );
...
bool
EarthManipulator2::recalculateCenterFromLookVector()
{
...
void
EarthManipulator2::recalculateCenter( const osg::CoordinateFrame& frame )
{
    osg::ref_ptr<osg::Node> node;
    if ( _node.lock(node) )
    {
...
void
EarthManipulator2::pan( double dx, double dy )
{
    if ( !isTethering() )
    {
...
void
EarthManipulator2::rotate( double dx, double dy )
{
...
void
EarthManipulator2::zoom( double dx, double dy )
{
...
bool
EarthManipulator2::screenToWorld(float x, float y, osg::View* theView, osg::Vec3d& out_coords) const
{
...
void
EarthManipulator2::setDistance( double distance )
{
    _distance = osg::clampBetween( distance, _settings->getMinDistance(), _settings->getMaxDistance() );
}

void
EarthManipulator2::setInitialVFOV(double vfov)
{
    _lastKnownVFOV = vfov;
}

void
EarthManipulator2::dumpActionInfo( const EarthManipulator2::Action& action, osg::NotifySeverity level ) const
{
...
void
EarthManipulator2::handleMovementAction( const ActionType& type, double dx, double dy, osg::View* view )
{
    switch( type )
    {
    case ACTION_PAN:
...
bool
EarthManipulator2::handlePointAction( const Action& action, float mx, float my, osg::View* view )
{
...
//                here.setFocalPoint( pointVP );
            ...
            break;
            case ACTION_MOUSE_ROTATE:
            {
                
                std::cout << "ACTION_MOUSE_ROTATE" << std::endl;
                std::cout << "select point: " << point.x() <<"," << point.y() << "," << point.z() << std::endl;
                Viewpoint here = getViewpoint();
                here.focalPoint()->fromWorld(_srs.get(), point);
                std::cout << "select point: " << point.x() <<"," << point.y() << "," << point.z() << std::endl;
                _mousePoint = point;
              ...
              osg::ref_ptr<osg::EllipsoidModel> elpModel=new osg::EllipsoidModel;
               elpModel->convertXYZToLatLongHeight(
                point.x(),point.y(),point.z(),firstlat,firstlon,firsthei);
               double tempx,tempy,tempz;

               std::cout << "EllipsoidModel:" << firstlat <<"," <<firstlon <<"," << firsthei << std::endl;
               elpModel->convertLatLongHeightToXYZ(

                firstlat,firstlon,firsthei+1000,tempx,tempy,tempz);
    ...
    {  // inverse world matrix 
         ...
         _moveoffset = (_center - _mousePoint);
        /* _viewOffset.x() = _viewOffset.x() + offset.x();
         _viewOffset.y() = _viewOffset.y() + offset.y();*/
        // _center += _moveoffset;
           _viewOffset += osg::Vec2(_moveoffset.x(),_moveoffset.y());
    }
...
void
EarthManipulator2::handleContinuousAction( const Action& action, osg::View* view )
{
...
void
EarthManipulator2::applyOptionsToDeltas( const Action& action, double& dx, double& dy )
{
...
bool
EarthManipulator2::handleMouseAction( const Action& action, osg::View* view )
{
...
bool
EarthManipulator2::handleMouseClickAction( const Action& action )
{
    //TODO.
    return false;
}
...
bool
EarthManipulator2::handleKeyboardAction( const Action& action, double duration )
{
    double dx = 0, dy = 0;
...
bool
EarthManipulator2::handleScrollAction( const Action& action, double duration )
{
...
bool
EarthManipulator2::handleAction( const Action& action, double dx, double dy, double duration )
{
    bool handled = true;
...
void
EarthManipulator2::recalculateRoll()
{
    osg::Matrixd rotation_matrix;
...
void
EarthManipulator2::getCompositeEulerAngles( double* out_azim, double* out_pitch ) const
{
...
void
EarthManipulator2::getEulerAngles(const osg::Quat& q, double* out_azim, double* out_pitch) const
{
    osg::Matrix m( q );
...
osg::Quat
EarthManipulator2::getQuaternion(double azim, double pitch) const
{
...
void
EarthManipulator2::collapseTetherRotationIntoRotation()
{
...
void
EarthManipulator2::setHomeViewpoint( const Viewpoint& vp, double duration_s )
{
    _homeViewpoint = vp;
...
// rotation. EarthManipulator2 represents the placement matrix as the
...
void
EarthManipulator2::drag(double dx, double dy, osg::View* theView)
{
    osgViewer::View* view = dynamic_cast<osgViewer::View*>(theView);
    if ( !view )
        return;
...
void
EarthManipulator2::updateMouseRotate()
{
    // 更新相机旋转
    osg::Matrix rotation_matrix;
    rotation_matrix.makeRotate(_rotation);
    // opengl坐标系下
    osg::Vec3d lookVector = -getUpVector(rotation_matrix); // -z 方向为视线默认方向
    osg::Vec3d sideVector = getSideVector(rotation_matrix); // +x 方向
    osg::Vec3d upVector = getFrontVector(rotation_matrix); // y 为up方向

    osg::Vec3 front = lookVector;
    osg::Quat r1 = _rotation; // old rotate

    osg::Quat r2 = osg::Quat(2 * osg::PI / 90.0, front);
    _rotation = (r1 * r2);

    osg::Matrixd m = getWorldMatrix()* osg::Matrix::translate(-_moveoffset);
   // setByMatrix(m);
   

   // double angle = frontDir.dot(pt1 - pt2);
    std::cout << "center:" << _center.x() <<"," <<_center.y() <<"," <<_center.z() << std::endl;

    //std::cout << "geoPoint: "<< _centerMap.x() <<"," <<_centerMap.y() <<"," <<_centerMap.alt() << std::endl;
}
